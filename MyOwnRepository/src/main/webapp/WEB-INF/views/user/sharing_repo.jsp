...
<body>
	...
	<br><br>
	<c:choose>
		<c:when test="${is_SCrepo == '0'}">
			<p>공유저장소</p>
		</c:when>
		<c:when test="${is_SCrepo == '1'}">
			<p>비밀저장소</p>
		</c:when>
	</c:choose>
	...
	<c:choose>
		...
		<c:when test="${member.id != null}">
			<c:choose>
				<c:when test="${is_SCrepo == '0'}">
					<input type="button" onclick="location.href='/user/write_board/s0r1'" class="write_button" value="글쓰기">
				</c:when>
				<c:when test="${is_SCrepo == '1'}">
					<input type="button" onclick="location.href='/user/write_board/s1r1'" class="write_button" value="글쓰기">
				</c:when>
			</c:choose>
		</c:when>
	</c:choose>
	...
	<div class="bottom_div">
		...
		<c:if test="${first_page > 5}">
			<a href="/user/sharingRepo/${first_page-5}"><b>&lt;&nbsp;이전</b></a>&emsp;
		</c:if>
		<div class="paging_div">
			...
			<c:forEach begin="${first_page}" end="${last_page}" var="p">
				<c:choose>
					...
					<c:when test="${p != select_page}">
						<a href="/user/sharingRepo/${p}">${p}</a>&emsp;
					</c:when>
				</c:choose>
			</c:forEach>
			</div>
		...
		<c:if test="${last_page != page_count}">
			&emsp;
			<a href="/user/sharingRepo/${last_page+1}"><b>다음&nbsp;&lt;</b></a>
		</c:if>
		<hr class="hr_sty">
